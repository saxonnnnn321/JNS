...
/** A customer this quote already belongs to, when started from their page. */
export type QuoteFor = {
  id: string;
  name: string;
  phone: string;
  email: string;
  properties: { id: string; label: string; lookup: string }[];
};

export function QuoteForm({ quoteFor }: { quoteFor?: QuoteFor }) {
  const [customer, setCustomer] = useState({
    name: quoteFor?.name ?? '',
    phone: quoteFor?.phone ?? '',
    email: quoteFor?.email ?? '',
  });
            ...
            {quoteFor && quoteFor.properties.length > 0 && (
              <div className="mt-3">
                ...
                <div className="mt-1 flex flex-wrap gap-2">
                  {quoteFor.properties.map((property) => (
                    <button
                      key={property.id}
                      type="button"
                      onClick={() => {
                        setAddressQuery(property.lookup);
                        void findProperty(property.lookup);
                      }}
                      className="rounded-lg border border-leaf/40 px-3 py-1.5 text-xs text-leaf hover:bg-leaf-soft"
                    >
                      {property.label}
                    </button>
                  ))}
